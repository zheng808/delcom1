<div id="results">
  <?php $base_url = 'timelogs/workorderitemselect?id='.$workorder->getId().'&parent_id='; ?>
  ...
  <?php if(isset($path)): ?>
    ...
    <div class="woiselect-thistask">
      <?php echo link_to('Select "'.$parent->getLabel().'" As the Task', 'timelogs/details?id='.$workorder->getId().'&item='.$parent->getId(),
                         array('class' => 'button')); ?>                
      <div class="clear"></div>
    </div>
  ...
  <?php
    ...
    if ($page > 1 && count($children) > 0)
    {
      ...
      $children = array_slice($children, $offset - 1 ); //zero-indexed
    }
  ?>
  <?php if (count($children) > 0): ?>
    ...
    <?php foreach ($children AS $idx => $child): ?>
      ...
      <?php endif; ?>
        ...
        <?php else: ?>
          <?php echo link_to(($offset + $idx).': '.$child->getLabel(), 'timelogs/details?id='.$workorder->getId().'&item='.$child->getId(), 
