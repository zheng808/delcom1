...
<?php 
  ...
  function updateLabour(labour_id){
    ...
    $$('.nonbill-item a').each(function(el,idx){
...
"); ?>
<div id="filter">
  <h2>Add Non-Billable Timelog</h2>
</div>

<?php echo form_tag('timelogs/nonbillable'); ?>
<input type="hidden" id="labour_type_id" name="labour_type_id" value="<?php echo $sf_request->getParameter('labour_type_id'); ?>" />
  ...
  <div class="labour-area rounded10">
    <h2 class="rounded10-top">Non-Bill Type</h2>
    <?php foreach ($labours AS $labour): ?>
      <div class="nonbill-item">
        <?php echo link_to_function($labour, "updateLabour('".$labour."')", array('id' => 'labour_option_'.$labour, 'class' => 'button')); ?>
      </div>
    <?php endforeach; ?>
  </div>
  ...
  <div class="time-area rounded10">
    ...
    <div id="time_mins">
      <?php for ($i = 0; $i <= 55; $i += 5): ?>
        <?php echo link_to_function(':'.($i < 10 ? '0'.$i : $i), "updateTimeMinutes(".$i.")", array('class' => 'button', 'id' => 'minutes_'.$i)); ?>
      <?php endfor; ?>
    </div>
    <div class="clear"></div>
  ...
  <div class="notes-area rounded10">
    <h2 class="rounded10-top">Notes</h2>
    <p>If needed, include any relevant information here, using the keyboard.</p>
