...
/**
 * Base static class for performing query and update operations on the 'shipment' table.
 *
 * 
 *
...
abstract class BaseShipmentPeer {
	...
	const CLASS_DEFAULT = 'lib.model.Shipment';
	...
	const TRACKING_NUMBER = 'shipment.TRACKING_NUMBER';

	/** the column name for the DATE_SHIPPED field */
	const DATE_SHIPPED = 'shipment.DATE_SHIPPED';

	/** the column name for the INVOICE_ID field */
	const INVOICE_ID = 'shipment.INVOICE_ID';
	...
	private static $fieldNames = array (
		BasePeer::TYPE_PHPNAME => array ('Id', 'Carrier', 'TrackingNumber', 'DateShipped', 'InvoiceId', ),
		BasePeer::TYPE_STUDLYPHPNAME => array ('id', 'carrier', 'trackingNumber', 'dateShipped', 'invoiceId', ),
		BasePeer::TYPE_COLNAME => array (self::ID, self::CARRIER, self::TRACKING_NUMBER, self::DATE_SHIPPED, self::INVOICE_ID, ),
		BasePeer::TYPE_FIELDNAME => array ('id', 'carrier', 'tracking_number', 'date_shipped', 'invoice_id', ),
	...
	private static $fieldKeys = array (
		BasePeer::TYPE_PHPNAME => array ('Id' => 0, 'Carrier' => 1, 'TrackingNumber' => 2, 'DateShipped' => 3, 'InvoiceId' => 4, ),
		BasePeer::TYPE_STUDLYPHPNAME => array ('id' => 0, 'carrier' => 1, 'trackingNumber' => 2, 'dateShipped' => 3, 'invoiceId' => 4, ),
		BasePeer::TYPE_COLNAME => array (self::ID => 0, self::CARRIER => 1, self::TRACKING_NUMBER => 2, self::DATE_SHIPPED => 3, self::INVOICE_ID => 4, ),
		BasePeer::TYPE_FIELDNAME => array ('id' => 0, 'carrier' => 1, 'tracking_number' => 2, 'date_shipped' => 3, 'invoice_id' => 4, ),
		BasePeer::TYPE_NUM => array (0, 1, 2, 3, 4, )
	);
	...
	public static function getMapBuilder()
	{
		if (self::$mapBuilder === null) {
			self::$mapBuilder = new ShipmentMapBuilder();
		}
	...
	public static function addSelectColumns(Criteria $criteria)
	{
		...
		$criteria->addSelectColumn(ShipmentPeer::CARRIER);

		$criteria->addSelectColumn(ShipmentPeer::TRACKING_NUMBER);

		$criteria->addSelectColumn(ShipmentPeer::DATE_SHIPPED);

		$criteria->addSelectColumn(ShipmentPeer::INVOICE_ID);
    ...
    foreach (sfMixer::getCallables('BaseShipmentPeer:doCount:doCount') as $callable)
    {
      call_user_func($callable, 'BaseShipmentPeer', $criteria, $con);
    }
    ...
    foreach (sfMixer::getCallables('BaseShipmentPeer:doSelectStmt:doSelectStmt') as $callable)
    {
      call_user_func($callable, 'BaseShipmentPeer', $criteria, $con);
    }
	...
	public static function doCountJoinInvoice(Criteria $criteria, $distinct = false, PropelPDO $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		...
		$criteria->addJoin(array(ShipmentPeer::INVOICE_ID,), array(InvoicePeer::ID,), $join_behavior);
	...
	/**
	 * Selects a collection of Shipment objects pre-filled with their Invoice objects.
	 * @param      Criteria  $c
	 * @param      PropelPDO $con
	 * @param      String    $join_behavior the type of joins to use, defaults to Criteria::LEFT_JOIN
    ...
    foreach (sfMixer::getCallables('BaseShipmentPeer:doSelectJoin:doSelectJoin') as $callable)
    {
      call_user_func($callable, 'BaseShipmentPeer', $c, $con);
    }
		...
		$c->addJoin(array(ShipmentPeer::INVOICE_ID,), array(InvoicePeer::ID,), $join_behavior);
		...
		while ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			...
			if ($key2 !== null) {
				...
				// Add the $obj1 (Shipment) to $obj2 (Invoice)
    ...
    foreach (sfMixer::getCallables('BaseShipmentPeer:doSelectJoinAll:doSelectJoinAll') as $callable)
    {
      call_user_func($callable, 'BaseShipmentPeer', $c, $con);
    }
		...
		while ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			...
			if ($key2 !== null) {
				...
				// Add the $obj1 (Shipment) to the collection in $obj2 (Invoice)
				$obj2->addShipment($obj1);
  ...
  static public function getUniqueColumnNames()
  {
	...
	{
		return ShipmentPeer::CLASS_DEFAULT;
	}
    ...
    foreach (sfMixer::getCallables('BaseShipmentPeer:doInsert:pre') as $callable)
    {
      $ret = call_user_func($callable, 'BaseShipmentPeer', $values, $con);
      if (false !== $ret)
      {
        return $ret;
      }
    ...
    foreach (sfMixer::getCallables('BaseShipmentPeer:doInsert:post') as $callable)
    {
      call_user_func($callable, 'BaseShipmentPeer', $values, $con, $pk);
    }
    ...
    foreach (sfMixer::getCallables('BaseShipmentPeer:doUpdate:pre') as $callable)
    {
      $ret = call_user_func($callable, 'BaseShipmentPeer', $values, $con);
      if (false !== $ret)
      {
		...
		if ($values instanceof Criteria) {
			...
			$selectCriteria->add(ShipmentPeer::ID, $criteria->remove(ShipmentPeer::ID), $comparison);
    ...
    foreach (sfMixer::getCallables('BaseShipmentPeer:doUpdate:post') as $callable)
    {
      call_user_func($callable, 'BaseShipmentPeer', $values, $con, $ret);
    }
	...
	/**
	 * Method to DELETE all rows from the shipment table.
	 *
	 * @return     int The number of affected rows (if supported by underlying database driver).
	 */
	public static function doDeleteAll($con = null)
	{
		...
		try {
			...
			$con->beginTransaction();
			$affectedRows += BasePeer::doDeleteAll(ShipmentPeer::TABLE_NAME, $con);
			$con->commit();
			return $affectedRows;
		} catch (PropelException $e) {
			$con->rollBack();
	 ...
	 */
	 public static function doDelete($values, PropelPDO $con = null)
	 {
		...
		} elseif ($values instanceof Shipment) {
	...
	public static function doValidate(Shipment $obj, $cols = null)
	{
		...
		$res =  BasePeer::doValidate(ShipmentPeer::DATABASE_NAME, ShipmentPeer::TABLE_NAME, $columns);
    if ($res !== true) {
        foreach ($res as $failed) {
            $col = ShipmentPeer::translateFieldname($failed->getColumn(), BasePeer::TYPE_COLNAME, BasePeer::TYPE_PHPNAME);
        }
    }
...
//
// NOTE: This static code cannot call methods on the ShipmentPeer class, because it is not defined yet.
// If you need to use overridden methods, you can add this code to the bottom of the ShipmentPeer class:
//
// Propel::getDatabaseMap(ShipmentPeer::DATABASE_NAME)->addTableBuilder(ShipmentPeer::TABLE_NAME, ShipmentPeer::getMapBuilder());
//
// Doing so will effectively overwrite the registration below.

Propel::getDatabaseMap(BaseShipmentPeer::DATABASE_NAME)->addTableBuilder(BaseShipmentPeer::TABLE_NAME, BaseShipmentPeer::getMapBuilder());
