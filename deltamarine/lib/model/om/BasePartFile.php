...
/**
 * Base class that represents a row from the 'part_file' table.
 *
 * 
 *
 * @package    lib.model.om
 */
abstract class BasePartFile extends BaseObject  implements Persistent {


  const PEER = 'PartFilePeer';
	...
	/**
	 * Initializes internal state of BasePartFile object.
	 * @see        applyDefaults()
	 */
	 ...
	 * @return     PartFile The current object (for fluent API support)
	 */
	public function setPartId($v)
	{
		...
		if ($this->part_id !== $v) {
			$this->part_id = $v;
			$this->modifiedColumns[] = PartFilePeer::PART_ID;
		}
	...
	public function setPartVariantId($v)
	{
		...
		if ($this->part_variant_id !== $v) {
			...
			$this->modifiedColumns[] = PartFilePeer::PART_VARIANT_ID;
		}
	...
	public function setFileId($v)
	{
		...
		if ($this->file_id !== $v) {
			$this->file_id = $v;
			$this->modifiedColumns[] = PartFilePeer::FILE_ID;
		}
	...
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = PartFilePeer::ID;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 4; // 4 = PartFilePeer::NUM_COLUMNS - PartFilePeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating PartFile object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = PartFilePeer::doSelectStmt($this->buildPkeyCriteria(), $con);
    ...
    foreach (sfMixer::getCallables('BasePartFile:delete:pre') as $callable)
    {
      ...
      if ($ret)
      {
		...
		try {
			PartFilePeer::doDelete($this, $con);
    ...
    foreach (sfMixer::getCallables('BasePartFile:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BasePartFile:save:pre') as $callable)
    {
      ...
      if (is_int($affectedRows))
      {
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BasePartFile:save:post') as $callable)
    {
      call_user_func($callable, $this, $con, $affectedRows);
    }

			PartFilePeer::addInstanceToPool($this);
			return $affectedRows;
		} catch (PropelException $e) {
			$con->rollBack();
			throw $e;
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = PartFilePeer::doInsert($this, $con);
					...
					$affectedRows += PartFilePeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = PartFilePeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = PartFilePeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = PartFilePeer::getFieldNames($keyType);
		$result = array(
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(PartFilePeer::PART_ID)) $criteria->add(PartFilePeer::PART_ID, $this->part_id);
		if ($this->isColumnModified(PartFilePeer::PART_VARIANT_ID)) $criteria->add(PartFilePeer::PART_VARIANT_ID, $this->part_variant_id);
		if ($this->isColumnModified(PartFilePeer::FILE_ID)) $criteria->add(PartFilePeer::FILE_ID, $this->file_id);
		if ($this->isColumnModified(PartFilePeer::ID)) $criteria->add(PartFilePeer::ID, $this->id);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(PartFilePeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of PartFile (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     PartFile Clone of current object.
	 * @throws     PropelException
	 */
	...
	public function getPartVariant(PropelPDO $con = null)
	{
		if ($this->aPartVariant === null && ($this->part_variant_id !== null)) {
			...
			/* The following can be used additionally to
			   ...
			   undesirable since it could result in an only partially populated collection
			   in the referenced object.
			   $this->aPartVariant->addPartFiles($this);
			 */
		}
  ...
  {
    if (!$callable = sfMixer::getCallable('BasePartFile:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BasePartFile::%s', $method));
    }
