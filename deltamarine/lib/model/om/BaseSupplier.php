...
abstract class BaseSupplier extends BaseObject  implements Persistent {
  ...
  const PEER = 'SupplierPeer';
	...
	/**
	 * The value for the credit_limit field.
	 * @var        string
	 */
	protected $credit_limit;

	/**
	 * The value for the net_days field.
	...
	/**
	 * Get the [credit_limit] column value.
	 * 
	 * @return     string
	 */
	...
	public function setWfCrmId($v)
	{
		...
		if ($this->wf_crm_id !== $v) {
			$this->wf_crm_id = $v;
			$this->modifiedColumns[] = SupplierPeer::WF_CRM_ID;
		}
	...
	public function setAccountNumber($v)
	{
		...
		if ($this->account_number !== $v) {
			...
			$this->modifiedColumns[] = SupplierPeer::ACCOUNT_NUMBER;
		}
	...
	/**
	 * Set the value of [credit_limit] column.
	 * 
	 * @param      string $v new value
	 * @return     Supplier The current object (for fluent API support)
	 */
	public function setCreditLimit($v)
	{
		...
		if ($this->credit_limit !== $v) {
			$this->credit_limit = $v;
			$this->modifiedColumns[] = SupplierPeer::CREDIT_LIMIT;
		}
	...
	/**
	 * Set the value of [net_days] column.
	 * 
	 * @param      int $v new value
	 * @return     Supplier The current object (for fluent API support)
	 */
	public function setNetDays($v)
	{
		if ($v !== null) {
			$v = (int) $v;
		}

		if ($this->net_days !== $v || $v === 0) {
			$this->net_days = $v;
			$this->modifiedColumns[] = SupplierPeer::NET_DAYS;
		}
	...
	public function setHidden($v)
	{
		...
		if ($this->hidden !== $v || $v === false) {
			$this->hidden = $v;
			$this->modifiedColumns[] = SupplierPeer::HIDDEN;
		}
	...
	public function hasOnlyDefaultValues()
	{
			...
			if (array_diff($this->modifiedColumns, array(SupplierPeer::NET_DAYS,SupplierPeer::HIDDEN))) {
				return false;
			}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->account_number = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			$this->credit_limit = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
			$this->net_days = ($row[$startcol + 4] !== null) ? (int) $row[$startcol + 4] : null;
    ...
    foreach (sfMixer::getCallables('BaseSupplier:delete:pre') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseSupplier:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseSupplier:save:pre') as $callable)
    {
      ...
      if (is_int($affectedRows))
      {
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BaseSupplier:save:post') as $callable)
    {
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true)
	{
		...
		$result = array(
			...
			$keys[3] => $this->getCreditLimit(),
			$keys[4] => $this->getNetDays(),
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[2], $arr)) $this->setAccountNumber($arr[$keys[2]]);
		if (array_key_exists($keys[3], $arr)) $this->setCreditLimit($arr[$keys[3]]);
		if (array_key_exists($keys[4], $arr)) $this->setNetDays($arr[$keys[4]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(SupplierPeer::WF_CRM_ID)) $criteria->add(SupplierPeer::WF_CRM_ID, $this->wf_crm_id);
		if ($this->isColumnModified(SupplierPeer::ACCOUNT_NUMBER)) $criteria->add(SupplierPeer::ACCOUNT_NUMBER, $this->account_number);
		if ($this->isColumnModified(SupplierPeer::CREDIT_LIMIT)) $criteria->add(SupplierPeer::CREDIT_LIMIT, $this->credit_limit);
		if ($this->isColumnModified(SupplierPeer::NET_DAYS)) $criteria->add(SupplierPeer::NET_DAYS, $this->net_days);
		if ($this->isColumnModified(SupplierPeer::HIDDEN)) $criteria->add(SupplierPeer::HIDDEN, $this->hidden);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setCreditLimit($this->credit_limit);

		$copyObj->setNetDays($this->net_days);
	 ...
	 * related PartSuppliers from storage. If this Supplier is new, it will return
		...
		if ($this->collPartSuppliers === null) {
			if ($this->isNew()) {
			   $this->collPartSuppliers = array();
				...
				$criteria->add(PartSupplierPeer::SUPPLIER_ID, $this->id);
	 ...
	 */
	public function getPartSuppliersJoinPartVariant($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(SupplierPeer::DATABASE_NAME);
		}
		...
		if ($this->collPartSuppliers === null) {
			if ($this->isNew()) {
				...
				$this->collPartSuppliers = PartSupplierPeer::doSelectJoinPartVariant($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this Supplier has previously been saved, it will retrieve
	 * related SupplierOrders from storage. If this Supplier is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->collSupplierOrders === null) {
			if ($this->isNew()) {
			   $this->collSupplierOrders = array();
				...
				$criteria->add(SupplierOrderPeer::SUPPLIER_ID, $this->id);
	 ...
	 * actually need in Supplier.
	 */
	public function getSupplierOrdersJoinInvoice($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(SupplierPeer::DATABASE_NAME);
		}
		...
		if ($this->collSupplierOrders === null) {
			if ($this->isNew()) {
				...
				$this->collSupplierOrders = SupplierOrderPeer::doSelectJoinInvoice($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this Supplier has previously been saved, it will retrieve
	 * related WorkorderItemBillables from storage. If this Supplier is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->collWorkorderItemBillables === null) {
			if ($this->isNew()) {
			   $this->collWorkorderItemBillables = array();
				...
				$criteria->add(WorkorderItemBillablePeer::SUPPLIER_ID, $this->id);
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this Supplier has previously been saved, it will retrieve
	 * related WorkorderPayments from storage. If this Supplier is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->collWorkorderPayments === null) {
			if ($this->isNew()) {
			   $this->collWorkorderPayments = array();
				...
				$criteria->add(WorkorderPaymentPeer::SUPPLIER_ID, $this->id);
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this Supplier has previously been saved, it will retrieve
	 * related Invoices from storage. If this Supplier is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->collInvoices === null) {
			if ($this->isNew()) {
			   $this->collInvoices = array();
				...
				$criteria->add(InvoicePeer::SUPPLIER_ID, $this->id);
  ...
  {
    if (!$callable = sfMixer::getCallable('BaseSupplier:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BaseSupplier::%s', $method));
    }
