...
/**
 * Base class that represents a row from the 'nonbill_type' table.
 *
 * 
 *
 * @package    lib.model.om
 */
abstract class BaseNonbillType extends BaseObject  implements Persistent {


  const PEER = 'NonbillTypePeer';
	...
	/**
	 * Initializes internal state of BaseNonbillType object.
	 * @see        applyDefaults()
	 */
	 ...
	 * @return     NonbillType The current object (for fluent API support)
	 */
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = NonbillTypePeer::ID;
		}
	...
	public function setName($v)
	{
		...
		if ($this->name !== $v) {
			$this->name = $v;
			$this->modifiedColumns[] = NonbillTypePeer::NAME;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 2; // 2 = NonbillTypePeer::NUM_COLUMNS - NonbillTypePeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating NonbillType object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = NonbillTypePeer::doSelectStmt($this->buildPkeyCriteria(), $con);
    ...
    foreach (sfMixer::getCallables('BaseNonbillType:delete:pre') as $callable)
    {
      ...
      if ($ret)
      {
		...
		try {
			NonbillTypePeer::doDelete($this, $con);
    ...
    foreach (sfMixer::getCallables('BaseNonbillType:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseNonbillType:save:pre') as $callable)
    {
      ...
      if (is_int($affectedRows))
      {
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BaseNonbillType:save:post') as $callable)
    {
      call_user_func($callable, $this, $con, $affectedRows);
    }

			NonbillTypePeer::addInstanceToPool($this);
			return $affectedRows;
		} catch (PropelException $e) {
			$con->rollBack();
			throw $e;
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = NonbillTypePeer::doInsert($this, $con);
					...
					$affectedRows += NonbillTypePeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = NonbillTypePeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = NonbillTypePeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = NonbillTypePeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
			$keys[1] => $this->getName(),
		);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(NonbillTypePeer::ID)) $criteria->add(NonbillTypePeer::ID, $this->id);
		if ($this->isColumnModified(NonbillTypePeer::NAME)) $criteria->add(NonbillTypePeer::NAME, $this->name);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(NonbillTypePeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of NonbillType (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     NonbillType Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     NonbillTypePeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new NonbillTypePeer();
		}
		return self::$peer;
	}
	 ...
	 * Otherwise if this NonbillType has previously been saved, it will retrieve
	 * related Timelogs from storage. If this NonbillType is new, it will return
		...
		if ($this->collTimelogs === null) {
			if ($this->isNew()) {
			   $this->collTimelogs = array();
				...
				$criteria->add(TimelogPeer::NONBILL_TYPE_ID, $this->id);
	 ...
	 * Otherwise if this NonbillType is new, it will return
	 * an empty collection; or if this NonbillType has previously
	 * been saved, it will retrieve related Timelogs from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in NonbillType.
	 */
  ...
  {
    if (!$callable = sfMixer::getCallable('BaseNonbillType:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BaseNonbillType::%s', $method));
    }
