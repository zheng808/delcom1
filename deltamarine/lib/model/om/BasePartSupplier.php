...
/**
 * Base class that represents a row from the 'part_supplier' table.
 *
 * 
 *
 * @package    lib.model.om
 */
abstract class BasePartSupplier extends BaseObject  implements Persistent {


  const PEER = 'PartSupplierPeer';

	/**
	 * The Peer class.
	 * Instance provides a convenient way of calling static methods on a class
	 * that calling code may not be able to identify.
	 * @var        PartSupplierPeer
	 */
	...
	/**
	 * The value for the supplier_sku field.
	 * @var        string
	 */
	protected $supplier_sku;
	...
	/**
	 * Initializes internal state of BasePartSupplier object.
	 * @see        applyDefaults()
	 */
	...
	/**
	 * Get the [supplier_sku] column value.
	 * 
	 * @return     string
	 */
	 ...
	 * @return     PartSupplier The current object (for fluent API support)
	 */
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = PartSupplierPeer::ID;
		}
	...
	public function setPartVariantId($v)
	{
		...
		if ($this->part_variant_id !== $v) {
			...
			$this->modifiedColumns[] = PartSupplierPeer::PART_VARIANT_ID;
		}
	...
	public function setSupplierId($v)
	{
		...
		if ($this->supplier_id !== $v) {
			$this->supplier_id = $v;
			$this->modifiedColumns[] = PartSupplierPeer::SUPPLIER_ID;
		}
	...
	/**
	 * Set the value of [supplier_sku] column.
	 * 
	 * @param      string $v new value
	 * @return     PartSupplier The current object (for fluent API support)
	 */
	public function setSupplierSku($v)
	{
		if ($v !== null) {
			$v = (string) $v;
		}

		if ($this->supplier_sku !== $v) {
			$this->supplier_sku = $v;
			$this->modifiedColumns[] = PartSupplierPeer::SUPPLIER_SKU;
		}
	...
	public function setNotes($v)
	{
		...
		if ($this->notes !== $v) {
			$this->notes = $v;
			$this->modifiedColumns[] = PartSupplierPeer::NOTES;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->supplier_id = ($row[$startcol + 2] !== null) ? (int) $row[$startcol + 2] : null;
			$this->supplier_sku = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
			...
			return $startcol + 5; // 5 = PartSupplierPeer::NUM_COLUMNS - PartSupplierPeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating PartSupplier object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = PartSupplierPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
    ...
    foreach (sfMixer::getCallables('BasePartSupplier:delete:pre') as $callable)
    {
      ...
      if ($ret)
      {
		...
		try {
			PartSupplierPeer::doDelete($this, $con);
    ...
    foreach (sfMixer::getCallables('BasePartSupplier:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BasePartSupplier:save:pre') as $callable)
    {
      ...
      if (is_int($affectedRows))
      {
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BasePartSupplier:save:post') as $callable)
    {
      call_user_func($callable, $this, $con, $affectedRows);
    }

			PartSupplierPeer::addInstanceToPool($this);
			return $affectedRows;
		} catch (PropelException $e) {
			$con->rollBack();
			throw $e;
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = PartSupplierPeer::doInsert($this, $con);
					...
					$affectedRows += PartSupplierPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = PartSupplierPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = PartSupplierPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = PartSupplierPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
			$keys[1] => $this->getPartVariantId(),
			$keys[2] => $this->getSupplierId(),
			$keys[3] => $this->getSupplierSku(),
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[3], $arr)) $this->setSupplierSku($arr[$keys[3]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(PartSupplierPeer::ID)) $criteria->add(PartSupplierPeer::ID, $this->id);
		if ($this->isColumnModified(PartSupplierPeer::PART_VARIANT_ID)) $criteria->add(PartSupplierPeer::PART_VARIANT_ID, $this->part_variant_id);
		if ($this->isColumnModified(PartSupplierPeer::SUPPLIER_ID)) $criteria->add(PartSupplierPeer::SUPPLIER_ID, $this->supplier_id);
		if ($this->isColumnModified(PartSupplierPeer::SUPPLIER_SKU)) $criteria->add(PartSupplierPeer::SUPPLIER_SKU, $this->supplier_sku);
		if ($this->isColumnModified(PartSupplierPeer::NOTES)) $criteria->add(PartSupplierPeer::NOTES, $this->notes);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(PartSupplierPeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of PartSupplier (or compatible) type.
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setSupplierSku($this->supplier_sku);
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     PartSupplier Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     PartSupplierPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new PartSupplierPeer();
		}
		return self::$peer;
	}
	...
	public function getPartVariant(PropelPDO $con = null)
	{
		if ($this->aPartVariant === null && ($this->part_variant_id !== null)) {
			...
			/* The following can be used additionally to
			   ...
			   $this->aPartVariant->addPartSuppliers($this);
			 */
		}
	...
	public function getSupplier(PropelPDO $con = null)
	{
		if ($this->aSupplier === null && ($this->supplier_id !== null)) {
			...
			/* The following can be used additionally to
			   ...
			   undesirable since it could result in an only partially populated collection
			   in the referenced object.
			   $this->aSupplier->addPartSuppliers($this);
			 */
		}
	 ...
	 * Otherwise if this PartSupplier has previously been saved, it will retrieve
	 * related Barcodes from storage. If this PartSupplier is new, it will return
		...
		if ($this->collBarcodes === null) {
			if ($this->isNew()) {
			   $this->collBarcodes = array();
				...
				$criteria->add(BarcodePeer::PART_SUPPLIER_ID, $this->id);
	 ...
	 * Otherwise if this PartSupplier is new, it will return
	 * an empty collection; or if this PartSupplier has previously
	 * been saved, it will retrieve related Barcodes from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in PartSupplier.
	 */
	public function getBarcodesJoinPartVariant($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(PartSupplierPeer::DATABASE_NAME);
		}
		...
		if ($this->collBarcodes === null) {
			if ($this->isNew()) {
				...
				$this->collBarcodes = BarcodePeer::doSelectJoinPartVariant($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
  ...
  {
    if (!$callable = sfMixer::getCallable('BasePartSupplier:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BasePartSupplier::%s', $method));
    }
