  ...
  const PEER = 'FilePeer';
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 3; // 3 = FilePeer::NUM_COLUMNS - FilePeer::NUM_LAZY_LOAD_COLUMNS).
	 ...
	 * related PartFiles from storage. If this File is new, it will return
		...
		if ($this->collPartFiles === null) {
			if ($this->isNew()) {
			   $this->collPartFiles = array();
				...
				$criteria->add(PartFilePeer::FILE_ID, $this->id);
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this File has previously been saved, it will retrieve
	 * related WorkorderItemFiles from storage. If this File is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->collWorkorderItemFiles === null) {
			if ($this->isNew()) {
			   $this->collWorkorderItemFiles = array();
				...
				$criteria->add(WorkorderItemFilePeer::FILE_ID, $this->id);
	 ...
	 * actually need in File.
	 */
	public function getWorkorderItemFilesJoinWorkorderItem($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(FilePeer::DATABASE_NAME);
		}
		...
		if ($this->collWorkorderItemFiles === null) {
			if ($this->isNew()) {
				...
				$this->collWorkorderItemFiles = WorkorderItemFilePeer::doSelectJoinWorkorderItem($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
