...
/**
 * Base class that represents a row from the 'workorder_item_photo' table.
 *
 * 
 *
 * @package    lib.model.om
 */
abstract class BaseWorkorderItemPhoto extends BaseObject  implements Persistent {


  const PEER = 'WorkorderItemPhotoPeer';

	/**
	 * The Peer class.
	 * Instance provides a convenient way of calling static methods on a class
	 * that calling code may not be able to identify.
	 * @var        WorkorderItemPhotoPeer
	 */
	...
	/**
	 * Initializes internal state of BaseWorkorderItemPhoto object.
	 * @see        applyDefaults()
	 */
	 ...
	 * @return     WorkorderItemPhoto The current object (for fluent API support)
	 */
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = WorkorderItemPhotoPeer::ID;
		}
	...
	public function setWorkorderItemId($v)
	{
		...
		if ($this->workorder_item_id !== $v) {
			...
			$this->modifiedColumns[] = WorkorderItemPhotoPeer::WORKORDER_ITEM_ID;
		}
	...
	public function setPhotoId($v)
	{
		...
		if ($this->photo_id !== $v) {
			$this->photo_id = $v;
			$this->modifiedColumns[] = WorkorderItemPhotoPeer::PHOTO_ID;
		}
	...
	public function setCreatedAt($v)
	{
		...
		if ( $this->created_at !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = WorkorderItemPhotoPeer::CREATED_AT;
			}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 4; // 4 = WorkorderItemPhotoPeer::NUM_COLUMNS - WorkorderItemPhotoPeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating WorkorderItemPhoto object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = WorkorderItemPhotoPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
    ...
    foreach (sfMixer::getCallables('BaseWorkorderItemPhoto:delete:pre') as $callable)
    {
      ...
      if ($ret)
      {
		...
		try {
			WorkorderItemPhotoPeer::doDelete($this, $con);
    ...
    foreach (sfMixer::getCallables('BaseWorkorderItemPhoto:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseWorkorderItemPhoto:save:pre') as $callable)
    {
    ...
    if ($this->isNew() && !$this->isColumnModified(WorkorderItemPhotoPeer::CREATED_AT))
    {
      $this->setCreatedAt(time());
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BaseWorkorderItemPhoto:save:post') as $callable)
    {
      call_user_func($callable, $this, $con, $affectedRows);
    }

			WorkorderItemPhotoPeer::addInstanceToPool($this);
			return $affectedRows;
		} catch (PropelException $e) {
			$con->rollBack();
			throw $e;
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = WorkorderItemPhotoPeer::doInsert($this, $con);
					...
					$affectedRows += WorkorderItemPhotoPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = WorkorderItemPhotoPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = WorkorderItemPhotoPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = WorkorderItemPhotoPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(WorkorderItemPhotoPeer::ID)) $criteria->add(WorkorderItemPhotoPeer::ID, $this->id);
		if ($this->isColumnModified(WorkorderItemPhotoPeer::WORKORDER_ITEM_ID)) $criteria->add(WorkorderItemPhotoPeer::WORKORDER_ITEM_ID, $this->workorder_item_id);
		if ($this->isColumnModified(WorkorderItemPhotoPeer::PHOTO_ID)) $criteria->add(WorkorderItemPhotoPeer::PHOTO_ID, $this->photo_id);
		if ($this->isColumnModified(WorkorderItemPhotoPeer::CREATED_AT)) $criteria->add(WorkorderItemPhotoPeer::CREATED_AT, $this->created_at);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(WorkorderItemPhotoPeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of WorkorderItemPhoto (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     WorkorderItemPhoto Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     WorkorderItemPhotoPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new WorkorderItemPhotoPeer();
		}
		return self::$peer;
	}
	...
	public function getWorkorderItem(PropelPDO $con = null)
	{
		if ($this->aWorkorderItem === null && ($this->workorder_item_id !== null)) {
			...
			/* The following can be used additionally to
			   ...
			   $this->aWorkorderItem->addWorkorderItemPhotos($this);
			 */
		}
	...
	public function getPhoto(PropelPDO $con = null)
	{
		if ($this->aPhoto === null && ($this->photo_id !== null)) {
			...
			/* The following can be used additionally to
			   ...
			   undesirable since it could result in an only partially populated collection
			   in the referenced object.
			   $this->aPhoto->addWorkorderItemPhotos($this);
			 */
		}
		return $this->aPhoto;
	}
  ...
  {
    if (!$callable = sfMixer::getCallable('BaseWorkorderItemPhoto:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BaseWorkorderItemPhoto::%s', $method));
    }
