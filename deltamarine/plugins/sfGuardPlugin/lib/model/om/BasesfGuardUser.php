...
abstract class BasesfGuardUser extends BaseObject  implements Persistent {
  ...
  const PEER = 'sfGuardUserPeer';
	 ...
	 * related Employees from storage. If this sfGuardUser is new, it will return
		...
		if ($this->collEmployees === null) {
			if ($this->isNew()) {
			   $this->collEmployees = array();
				...
				$criteria->add(EmployeePeer::GUARD_USER_ID, $this->id);
	 ...
	 */
	public function getEmployeesJoinwfCRM($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		}
		...
		if ($this->collEmployees === null) {
			if ($this->isNew()) {
				...
				$this->collEmployees = EmployeePeer::doSelectJoinwfCRM($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this sfGuardUser has previously been saved, it will retrieve
	 * related Customers from storage. If this sfGuardUser is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->collCustomers === null) {
			if ($this->isNew()) {
			   $this->collCustomers = array();
				...
				$criteria->add(CustomerPeer::GUARD_USER_ID, $this->id);
	 ...
	 * actually need in sfGuardUser.
	 */
	public function getCustomersJoinwfCRM($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		}
		...
		if ($this->collCustomers === null) {
			if ($this->isNew()) {
				...
				$this->collCustomers = CustomerPeer::doSelectJoinwfCRM($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
