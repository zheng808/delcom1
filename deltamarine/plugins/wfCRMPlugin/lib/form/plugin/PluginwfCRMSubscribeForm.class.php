 ...
 */
class PluginwfCRMSubscribeForm extends BasewfCRMForm
{
  ...
  public function configure()
  {
    ...
    $this->widgetSchema['public_notes']->setLabel('Notes');
    
    $this->widgetSchema['wf_crm_category_ref_list'] = new sfWidgetFormSelectCheckbox(array(
      'choices' => wfCRMCategoryPeer::retrieveAllTree()
    ));
    
    $this->validatorSchema['email'] = new sfValidatorEmail();
    
    $conf = sfConfig::get('app_wf_crm_plugin_subscribe_form', null);
    if ($conf)
    { 
  ...
  protected function doSave($con = null)
  {
    if ($this->getObject()->isNew())
    {
      ...
      $this->getObject()->setPrivateNotes('added via subscribe page on ' . format_date(time(), 'g'));
    }
    parent::doSave($con);
  }
}
